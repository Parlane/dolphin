...
void HLE_IPC_CreateVirtualFATFilesystem()
{
	...
	if (File::GetSize(cdbPath) < cdbSize)
	{
		...
		if (cdbFile.IsOpen())
		{
			...
			cdbFile.WriteBytes(cdbFAT, 0x4);
			// 20 MiB file
			cdbFile.Seek((s64)cdbSize - 1, SEEK_SET);
...
void CWII_IPC_HLE_Device_FileIO::EnqueueReply(u32 CommandAddress, s32 ReturnValue)
{
...
void CWII_IPC_HLE_Device_FileIO::FileProcessor()
{
	INFO_LOG(WII_IPC_FILEIO, "FileIO: FileProcessor entered %s (DeviceID=%08x)", m_Name.c_str(), m_DeviceID);
	std::string thread_name = StringFromFormat("FileIO::thread for %s", m_Name.c_str());
	...
	while (processing)
	{
		...
		{
			u32 _CommandAddress = cmd.command_address;
			u32 ReturnValue = FS_EACCESS;
			switch(cmd.type)
			{
				case ECommandType::COMMAND_READ:
				{
				...
				case ECommandType::COMMAND_WRITE:
				{
			...
			EnqueueReply(_CommandAddress, ReturnValue);
		}
		file_mutex.lock();
	}
	file_mutex.unlock();
	INFO_LOG(WII_IPC_FILEIO, "FileIO: FileProcessor exited %s (DeviceID=%08x)", m_Name.c_str(), m_DeviceID);
}
...
bool CWII_IPC_HLE_Device_FileIO::Open(u32 _CommandAddress, u32 _Mode)
{
	...
	if (File::Exists(m_filepath))
	{
		...
		processing = true;
#ifdef USE_ASYNC_FILEIO
		thread = std::thread(std::mem_fun(&CWII_IPC_HLE_Device_FileIO::FileProcessor), this);
#endif
	}
	else
	{
...
IOFile CWII_IPC_HLE_Device_FileIO::OpenFile()
{
	...
	return IOFile(m_filepath, open_mode);
}
...
{
#ifdef USE_ASYNC_FILEIO
	FileCommand cmd = {_CommandAddress, ECommandType::COMMAND_READ};
	file_queue.Push(std::move(cmd));
	file_event.Set();
	return false;
#else
	u32 ReturnValue = FS_EACCESS;
...
{
#ifdef USE_ASYNC_FILEIO
	FileCommand cmd = {_CommandAddress, ECommandType::COMMAND_WRITE};
	file_queue.Push(std::move(cmd));
	file_event.Set();
	return false;
#else
